...
namespace Parallel
{
    ...
    namespace detail
    {
        template <typename T, typename Op, typename U> 
            ...
            T reduce_seq(Op&& binary_op, const U &collection, size_t start, size_t end, T identity, int depth, int max_depth)
            {
                ...
                {
                    left = reduce_seq(std::forward<Op>(binary_op), collection, start, mid, identity, depth + 1, max_depth);
                }

                right = reduce_seq(std::forward<Op>(binary_op), collection, mid, end, identity, depth + 1, max_depth);
        ...
        T reduce(Op&& binary_op, const U &collection, T identity)
        {
            ...
            #pragma omp parallel
            {
                #pragma omp single nowait
                {
                    ...
                    result = detail::reduce_seq(std::forward<Op>(binary_op), 
                            collection, 
                            0, 
                            collection.size(), 
                            identity, 
