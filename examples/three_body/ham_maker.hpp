...
template<size_t Idx, double Mass>
struct BodyEntry {};
...
struct MakePairsWith {
    using type = Nil;
};

template<typename Head, typename H2, typename Tail>
struct MakePairsWith<Head, List<H2, Tail>> {
    using type = List<PairEntry<Head, H2>, typename MakePairsWith<Head, Tail>::type>;
};
...
template<typename Head, typename Tail>
struct MakeAllPairs<List<Head, Tail>> {
    using type = Concat_t<
        MakePairsWith_t<Head, Tail>,
        typename MakeAllPairs<Tail>::type
    >;
};

template<typename L>
using MakeAllPairs_t = typename MakeAllPairs<L>::type;
...
template<typename List, size_t N_Bodies>
struct MakeKineticPart 
{
};

template<size_t N_Bodies>
struct MakeKineticPart<Nil, N_Bodies>
{
...
template<size_t N_Bodies>
struct MakePotentialPart<Nil, N_Bodies>
{
    using type = Constant<0.0>;
};

template<size_t i, size_t j, double Mass_i, double Mass_j, typename Tail, size_t N_Bodies>
struct MakePotentialPart<List<PairEntry<BodyEntry<i,Mass_i>, BodyEntry<j, Mass_j>>, 
    Tail>, 
    N_Bodies>
{
    static decltype(auto) _make_part()
    {
        ...
        auto r = sqrt((_qiy-_qjy)*(_qiy-_qjy) + (_qix-_qjx)*(_qix-_qjx));
        return Constant<-1.0>{}/r;
    }
    using _part_type = decltype(_make_part());
    using type = Add<_part_type, typename MakePotentialPart<Tail, N_Bodies>::type>;
};

template<double... Masses>
decltype(auto) make_ham(Body<Masses>... args) {
