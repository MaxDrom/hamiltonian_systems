...
namespace RungeKutta
{
    ...
    BRK::BRK(int k, int s)
    {
        ...
        for (size_t i = 1; i <= roots1.size(); i++)
        {
            ...
            Newton::solve(roots1(i - 1), [&N](Real x)
                    { return Legandre(x, N); }, [&N](Real x)
                    { return DLegandre(x, N); });
    ...
    void BRK::Init(std::function<void(Real, const vector<Real> &, vector<Real> &)> rhs, int n)
    {
        _rhs = rhs;
        _first_step = true;
        _n = n;
