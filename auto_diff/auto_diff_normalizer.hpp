...
template<IsBinaryOpTag OpTag>
constexpr double combine(double a, double b) {
    return 0;
}

template<IsBinaryOpTag OpTag>
constexpr double identity() {
    return 0.0;
}

template<>
constexpr double combine<AddTag>(double a, double b) {
    return a+b;
}

template<>
constexpr double combine<MulTag>(double a, double b) {
    return a*b;
}

template<>
constexpr double identity<AddTag>() {
    return 0.0;
}

template<>
constexpr double identity<MulTag>() {
    return 1.0;
}
...
template <IsBinaryOpTag op, IsExpression Acc, IsNotNumber Head, typename Tail, double AccConstant>
struct FoldLeftList<op, Acc, TypeList<Head, Tail>, AccConstant> {
    using NewAcc = BinaryOp<Acc, Head, op>;
    using type = typename FoldLeftList<op, NewAcc, Tail, AccConstant>::type;
};

template <IsBinaryOpTag op, typename Acc, int Head, typename Tail, double AccConstant>
struct FoldLeftList<op, Acc, TypeList<Integer<Head>, Tail>, AccConstant> {
    using type = typename FoldLeftList<op, Acc, Tail, combine<op>((double)Head, AccConstant)>::type;
};

template <IsBinaryOpTag op, typename Acc, double Head, typename Tail, double AccConstant>
struct FoldLeftList<op, Acc, TypeList<Constant<Head>, Tail>, AccConstant> {
    using type = typename FoldLeftList<op, Acc, Tail, combine<op>(Head, AccConstant)>::type;
};

template <IsBinaryOpTag op, typename List>
struct FoldLeft {
