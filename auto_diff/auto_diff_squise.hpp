...
using Squise = ExpressionIterator<Squiser, A>;

template <typename Current, typename Next, bool IsSame>
...
struct FixedPointImpl<Current, Next, false> {
    ...
    using type = typename FixedPointImpl<
        Next, 
        _squized,
        std::is_same_v<Next, _squized>
    >::type;
};

template <IsExpression T>
struct FixedPointSquise {
    using FirstPass = typename Squise<typename Normalize<T>::type>::type;
    
    using type = typename FixedPointImpl<
        T, 
        FirstPass, 
        std::is_same_v<T, FirstPass>
    >::type;
};
