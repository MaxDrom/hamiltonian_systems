...
struct BinaryOpTag 
{
    ...
    inline std::string get_char() const
    {
        return pimpl()->get_char_impl();
    }

    inline size_t priority() const
    {
        return pimpl()->priority_impl();
    }
};
...
struct Function: Expression<Function<A, FuncTag>>
{
    ...
    inline Real apply_impl(Args&&... args) const
    {
...
using Sin = Function<Value, FuntionTagFromFunc<static_cast<Real(*)(Real&&)>(mp_sin)>>;

template<IsExpression Value>
static inline Sin<Value> sin(const Value& v)
{
    return {};
}

template<IsExpression Value>
using Cos = Function<Value, FuntionTagFromFunc<static_cast<Real(*)(Real&&)>(mp_cos)>>;

template<IsExpression Value>
static inline Cos<Value> cos(const Value& v)
{
    return {};
}

template<IsExpression Value>
using Sqrt = Function<Value, FuntionTagFromFunc<static_cast<Real(*)(Real&&)>(mp_sqrt)>>;

template<IsExpression Value>
static inline Sqrt<Value> sqrt(const Value& v)
{
    return {};
}
...
struct BinaryOp: public Expression<BinaryOp<Left, Right, OpTag>>
{
    ...
    inline Real apply_impl(Args&&... args) const
    {
        return _op.apply(_left(std::forward<Args>(args)...), _right(std::forward<Args>(args)...));
    }
    inline std::string print_impl() const
    {
        auto left = _op.priority() >= _left.priority() ? std::format("({})", _left.print()) : _left.print();
        auto right = _op.priority() >= _right.priority() ? std::format("({})", _right.print()) : _right.print();
...
struct MulTag : public BinaryOpTag<MulTag>
{
    ...
    private:
    inline Real apply_impl(Real a, Real b) const
    {
        return a*b;
    }
    inline std::string get_char_impl() const
    {
        return "*";
    }
...
using Mul = BinaryOp<A, B, MulTag>;

template<IsExpression A, IsExpression B>
constexpr static inline Mul<A,B> operator*(const Expression<A>& a, const Expression<B>& b)
{
    return {};
};

template<IsExpression A, IsExpression B>
using Div = BinaryOp<A, B, DivTag>;

template<IsExpression A, IsExpression B>
constexpr static inline Mul<A,Div<Constant<1.0>,B>> operator/(const Expression<A>& a, const Expression<B>& b)
{
    return {};
};

template<auto Func, IsExpression A>
constexpr static inline Function<A, FuntionTagFromFunc<Func>> apply_function(A a)
{
    return {};
}

template<IsExpression A, IsExpression B>
using Add = BinaryOp<A, B, AddTag>;


template<IsExpression A, IsExpression B>
constexpr static inline Add<A,B> operator+(const Expression<A>& a, const Expression<B>& b)
{
    return {};
};
...
template<IsExpression A, IsExpression B>
constexpr static inline Add<A,Mul<Constant<-1.0>,B>> operator-(const Expression<A>& a, const Expression<B>& b)
{
    return {};
};
...
template<template<typename...> class NodeWorker, IsExpression A, typename... Args>
struct ExpressionIterator
{
    using type = typename NodeWorker<A, Args...>::type;
};

template<template<typename...> class NodeWorker, IsExpression Left, IsExpression Right, IsBinaryOpTag OpTag, typename... Args>
struct ExpressionIterator<NodeWorker, BinaryOp<Left, Right, OpTag>, Args...>
{
    ...
    using _Right = typename ExpressionIterator<NodeWorker, Right, Args...>::type;

    using type = typename NodeWorker<BinaryOp<_Left, _Right, OpTag>, Args...>::type;
};

template<template<typename...> class NodeWorker, IsExpression A, IsFunctionTag FuncTag, typename... Args>
struct ExpressionIterator<NodeWorker, Function<A,FuncTag>, Args...> {
    using SimplifiedV = typename ExpressionIterator<NodeWorker, A, Args...>::type;
    
    using type = typename NodeWorker<Function<SimplifiedV, FuncTag>, Args...>::type;
};

template <size_t N_iters, IsExpression Expr, IsExpression Pattern, IsExpression Initial>
constexpr decltype(auto) recursive_subst(Expr expr, Pattern pattern, Initial initial)
{
