...
struct Diff<Div<A,B>, Var> 
{
    ...
    using type = Mul<Minus<Mul<_A,B>, Mul<A,_B>>,Div<Constant<1.0>,Mul<B,B>>>;
};

template<double CC, size_t Idx>
struct Diff<Constant<CC>, Variable<Idx>> 
{
    using type = Integer<0>;
};

template<int CC, size_t Idx>
struct Diff<Integer<CC>, Variable<Idx>> 
{
    using type = Integer<0>;
};
...
template<size_t Idx>
struct Diff<Variable<Idx>, Variable<Idx>>
{
    using type = Integer<1>;
};

template<size_t Idx, size_t Idx2> requires (Idx != Idx2)
...
template<IsExpression A, auto Func>
struct AnalyticalDiff {};

template <typename A, typename Var, auto Func>
concept HasAnalyticalDiff = requires {
    typename AnalyticalDiff<A,Func>::type;
};
...
template<IsExpression A, IsVariable Var>
FixedPointSquise<typename Diff<A, Var>::type>::type make_diff(const A& a, const Var& b)
{
    return  {};
};
