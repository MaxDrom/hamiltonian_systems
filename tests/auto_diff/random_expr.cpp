...
template <unsigned long long Seed, int Depth> struct RandomExpr {
    static constexpr unsigned long long r = CTRand<Seed>::value;
    static constexpr int choice = (r >> 32) % 4;

    using type =
        std::conditional_t<Depth == 0, Variable<0>,
...
struct RandomChoices<Seed, Depth, 0> {
    ...
    using type =
        Mul<typename RandomExpr<next_seed, Depth - 1>::type,
...
struct RandomChoices<Seed, Depth, 1> {
    using type =
        Sin<typename RandomExpr<CTRand<Seed>::next_seed, Depth - 1>::type>;
};

template <unsigned long long Seed, int Depth>
struct RandomChoices<Seed, Depth, 2> {
    using type =
        Cos<typename RandomExpr<CTRand<Seed>::next_seed, Depth - 1>::type>;
};

template <unsigned long long Seed, int Depth>
struct RandomChoices<Seed, Depth, 3> {
    using type =
        Exp<typename RandomExpr<CTRand<Seed>::next_seed, Depth - 1>::type>;
};

template <unsigned long long Seed> struct RandomExpr<Seed, 0> {
