...
namespace Newton
{
    ...
    void jacobi(matrix<Real> &jac, const vector<Real> &x, std::function<void(const vector<Real> &, vector<Real>&)> f, const vector<Real> &f_x)
    {
...
int solve_linear(const matrix<Real> &A, 
                 const vector<Real> &b, 
                 vector<Real> &x)
{
    ...
    using EigenMatrixMapConst = Eigen::Map<const Eigen::Matrix<Real, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>;
    // Типы для маппинга векторов
    using EigenVectorMapConst = Eigen::Map<const Eigen::Matrix<Real, Eigen::Dynamic, 1>>;
    using EigenVectorMap      = Eigen::Map<Eigen::Matrix<Real, Eigen::Dynamic, 1>>;

    // Отображаем память Boost в структуры Eigen без накладных расходов на копирование
    ...
    Eigen::CompleteOrthogonalDecomposition<Eigen::Matrix<Real, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> cod(eigen_A);

    // Находим псевдорешение с минимальной нормой и пишем напрямую в память вектора x
    ...
    int solve(vector<Real> &x, std::function<void(const vector<Real> &, vector<Real>&)> f,Real damping)
    {
    ...
    int solve(Real &x, std::function<Real(Real)> f, std::function<Real(Real)> df)
    {
        ...
        do
        {
            ...
            x = x+dx;
            //std::cout<<x<<" "<<dx<<'\n';
        } while (abs(fx) > std::numeric_limits<Real>::min() && abs(dx) > std::numeric_limits<Real>::epsilon());
